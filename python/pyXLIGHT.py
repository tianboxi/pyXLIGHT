...
class xfoilAnalysis():
    def __init__(self, airfoil_file, re=1e5,mach=0.0,iter=50):
        try:
            ...
            print( 'There was an error opening the airfoil file %s'.format(airfoil_file))
            sys.exit(1)
        # end if
        ...
        y = []
        for line in f:
            x.append(float(str.split(line)[0]))
            y.append(float(str.split(line)[1]))
        # end if
        self.x = array(x)
        self.y = array(y)
    ...
    def setValue(self,common_block,variable,value):
        ...
        if has_attribute:
            ...
            print( 'Either {0:s} or {1:s} does not exist'.format(common_block,variable))
        # end if
        return
